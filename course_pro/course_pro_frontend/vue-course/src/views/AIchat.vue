      ...
      <el-container class="main-container">
        ...
        <el-main class="main-content">
          <div class="card-container">
            ...
            <!-- "Course selection suggestions" 按钮 -->
            ...
            <!-- "Study suggestions for selected courses" 按钮 -->
...
const handleExit = () => {
  ...
  router.push('/course'); // 跳转到主页
};

const startChat = () => {
  router.push('/AIchat2'); // 将路径修改为 /AIchat2 以跳转到 AIchat2 页面
};
...
.feature-icon {
  width: 40px; /* 设置中间按钮图标的宽度 */
  height: 40px; /* 设置中间按钮图标的高度 */
  margin-bottom: 10px; /* 增加图标和文字之间的间距 */
  filter: brightness(0.8) contrast(1.2); /* 使图标颜色加深，增加对比度和降低亮度 */
}
...
.card-container {
  ...
  gap: 30px;
  flex-wrap: wrap;
  margin-top: -150px; /* 使用正的 margin 值 */
}
...
.feature-card {
  ...
  padding: 20px;
  cursor: pointer;
  border-radius: 30px; /* 增大圆角，使按钮更加圆润 */
  transition: transform 0.3s, box-shadow 0.3s;
  box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);
  font-weight: bold; /* 使文字加粗 */
  margin-top: -10px; /* 使用负的 margin 值将文字位置上移 */
}
...
.message-input-container {
  width: 100%;
  border-radius: 30px; /* 设置输入框容器的圆角 */
  overflow: hidden; /* 确保子元素的圆角生效 */
  border: 5px solid #409eff; /* 增加蓝色边框 */
}
